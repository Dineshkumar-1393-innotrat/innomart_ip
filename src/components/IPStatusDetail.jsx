...
function IPStatusDetail() {
  ...
  // NEW: local state for success message
  const [posted, setPosted] = useState(false);

  const handleProceedToSell = () => {
    if (!isApproved) return;
    // here you can call API to post the listing if needed
    setPosted(true); // just show success, no redirect
  };
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-xl p-6 sm:p-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm sm:text-base text-gray-600">
            Track the progress of your intellectual property listing.
          </p>
        </div>
        ...
        <div className="bg-blue-50 rounded-2xl p-5 sm:p-7 mb-8">
          ...
          <p className="text-sm sm:text-base text-gray-700 leading-relaxed">
            Description of IP – "IP" can refer to Internet Protocol or Intellectual Property,
            depending on the context. Internet Protocol (IP) is a set of rules and standards
            for sending data over networks, while an IP address is the unique numerical label
            assigned to devices. On the other hand, Intellectual Property (IP) encompasses
        ...
        <div>
          ...
          <div className="relative">
            <div
              className="absolute left-4 top-4 bottom-4 border-l-2 border-gray-200"
              aria-hidden="true"
            ></div>
            <div className="space-y-6 sm:space-y-8">
              {statusTimeline.map((step, index) => (
        ...
        {/* Success message (only after click) */}
        {posted && (
          <div className="mt-8 p-4 rounded-xl bg-green-50 border border-green-200 text-sm text-green-800">
            <span className="font-semibold">Posted successfully!</span> Your IP is now listed for sale.
          </div>
        )}
        ...
        <div className="mt-10 flex flex-col sm:flex-row justify-end gap-3">
          ...
          >
            {posted ? 'Posted Successfully' : 'Proceed To Sell'}
          </button>
        </div>
      </div>
    </div>
